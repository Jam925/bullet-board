...
app.get('/post/:id', (req, res) => {
    ...
    connection.query(postQuery, [post_id], (error, postResults) => {
        connection.query(commentsQuery, [post_id], (error, commentResults) => {
            ...
            res.render('detail_page', { post: postResults[0], comments: commentResults, user_id: req.session.user_id });
        });
    });
});
...
app.post('/likePost', (req, res) => {
    if (!req.session.user_id) {
        ...
        const updateLikesQuery = 'UPDATE posts SET likes = likes + 1 WHERE post_id = ?';
        
        connection.query(checkLikeQuery, [post_id, user_id], (error, results) => {
            if (results.length === 0) {
                connection.query(likeQuery, [post_id, user_id], (error, results) => {
                    connection.query(updateLikesQuery, [post_id], (error, results) => {
                        res.redirect(`/post/${post_id}`);
                    });
                });
            } else {
